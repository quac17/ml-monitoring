...
A machine learning monitoring stack using MLflow, Prometheus, Grafana, and Evidently for model observability and drift signals.
...
- **Database**: PostgreSQL for MLflow backend store and (optional) Airflow metadata
- **Orchestration (optional)**: Apache Airflow with StatsD → Prometheus for scheduler/webserver metrics
...
│       ▲                 │                  │                                │
│       │                 │         ┌────────┴──────┐                         │
│       │                 │         │  Evidently    │  capture /analyze        │
│       │                 │         │  :8001        │  → /metrics             │
│       │                 │         └───────────────┘                         │
│       │                 │                  ▲                                  │
│       │                 │         ┌────────┴────────┐                       │
│       │                 └────────▶│ statsd-exporter │◀── Airflow (StatsD)   │
│       │                           │  :9102 / :9125  │    :8080 (UI)         │
│       │                           └─────────────────┘                       │
│       │            ┌──────────┐                                             │
│       └───────────▶│  MLflow  │◀────────── (training host + API load model)   │
│                    │  :5000   │                                             │
│                    └────┬─────┘                                             │
│                         │                                                   │
│              ┌──────────┴──────────┐                                        │
│         ┌────▼─────┐         ┌────▼────┐                                   │
│         │PostgreSQL│         │  MinIO  │  (MLflow backend + artifacts)      │
│         │  :5432   │         │  :9000  │                                   │
│         └──────────┘         └─────────┘                                   │
└────────────────────────────────────────────────────────────────────────────┘
```

Training runs **on the host** (`scripts/training.py`): it pushes runs and registry artifacts to MLflow; the API container loads `models:/<MODEL_NAME>/<MODEL_STAGE>` via `mlflow.pyfunc`.
...
| **Evidently** | 8001 | Drift / data-quality service (Prometheus + HTML reports) |
| **Prometheus** | 9090 | Metrics collection (API, Evidently, StatsD/Airflow) |
| **Grafana** | 3000 | Dashboards (API + Evidently drift) |
| **MinIO** | 9000 | S3-compatible storage for MLflow artifacts |
| **MinIO Console** | 9001 | MinIO web UI |
| **PostgreSQL** | 5432 | MLflow backend; optional second DB `airflow` for Airflow |
| **StatsD Exporter** | 9102 (HTTP), 9125 (UDP) | Airflow metrics → Prometheus job `airflow` |
| **Airflow** | 8080 (UI) | Optional DAGs under `airflow_dags/` |
...
**Default configuration** (see `.env.example`):
- `USER` — prefix for container names (example: `hoangtv`)
- Dev passwords: MinIO, Grafana, Airflow admin — as listed in `.env.example`
...
1. Trains a RandomForest classifier on the sklearn Wine dataset
2. Logs model, parameters, and metrics to MLflow
3. Registers the model in the MLflow Model Registry
4. Promotes the version to **Production**
5. Stores artifacts in the MinIO bucket used by MLflow
...
```bash
# Optional: Grafana drift dashboard file / connectivity checks (bash)
chmod +x scripts/test_dashboard.sh scripts/test_evidently.sh
./scripts/test_dashboard.sh
./scripts/test_evidently.sh

# Traffic + optional Evidently capture/analysis
cd simulations
pip install -r requirements.txt
python run_simulation.py -n 200 -s moderate_drift --analyze
# or: bash quick_test.sh   (short smoke test)
cd ..
```

This sends traffic to `/predict`, updates API Prometheus metrics, and (when Evidently is up) can capture data and run drift analysis for Grafana.
...
| **Airflow Web UI** | http://localhost:8080 | Set via `AIRFLOW_ADMIN_*` in `.env` |
...
```bash
# User Configuration
USER=yourname                  # Container name prefix (see .env.example)
...
The training script (`scripts/training.py`) trains a **RandomForestClassifier** on **`sklearn.datasets.load_wine`** (13 features, **3-class** cultivar classification). The registered model name in MLflow is `wine_quality_model` (historical name in this repo).
...
**Request** (13 floats in **sklearn `load_wine` feature order**; `feature_names` optional but enables feature histograms in Prometheus when length matches):
...
`prediction` is the **predicted class** (0, 1, or 2) for the default training script.
...
```
ml-monitoring/
├── api/                       # FastAPI model serving + /metrics
├── mlflow/                    # MLflow server image
├── evidently/                 # Drift / data-quality service
├── simulations/               # Traffic & drift simulations (CLI)
├── airflow_dags/              # DAGs mounted into Airflow
├── scripts/                   # training.py, optional test_*.sh helpers
├── config/
│   ├── prometheus.yml         # Scrape jobs (api, evidently, airflow, …)
│   ├── prometheus/            # Rule files (e.g. evidently_alerts.yml)
│   ├── statsd_mapping.yml     # Airflow StatsD → Prometheus
│   └── grafana/               # Provisioning + dashboard JSON
├── docker-compose.yml
├── .env.example
├── HUONG_DAN_CHAY_DU_AN.md    # Vietnamese runbook
├── MO_TA_HOAT_DONG_DU_AN.md   # Vietnamese architecture / model flow
└── README.md
```
